...
TSQL_ROWOPS = re.compile(r'(^|[\(\s]+)(?P<rowop>INTERSECT|EXCEPT|UNION ALL|UNION)($|[\)\s]+)', flags=re.IGNORECASE | re.MULTILINE)
# TSQL_VARTABLE_NAMED = re.compile(r'(?=([\"\[]?(?P<table>[<>@A-Za-z0-9_]+)[\"\]]?\.[\"\[]?(?P<varname>[<>@A-Za-z0-9_]+|[\*])[\"\]]?(\s+AS\s+|\s)[\"\']?(?P<alias>[<>@A-Za-z0-9_]+)[\"\']?\s*(\s|,|$)))', flags=re.IGNORECASE | re.MULTILINE)
TSQL_VARTABLE_NAMED = re.compile(r'([\"\[]?(?P<table>[<>@A-Za-z0-9_]+)[\"\]]?\.[\"\[]?(?P<varname>[<>@A-Za-z0-9_]+|[\*])[\"\]]?(\s+AS\s+|\s)[\"\']?(?P<alias>[<>@A-Za-z0-9_]+)[\"\']?\s*(\s|,|$))', flags=re.IGNORECASE | re.MULTILINE)
